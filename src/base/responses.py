    ...
    def get_base_statuses(cls) -> dict:
        ...
        return {
            ...
            status.HTTP_422_UNPROCESSABLE_ENTITY: {
                'model': ExceptionValidationSchema
            },
        }

    def statuses(self, schema, response_status: int = status.HTTP_200_OK,
        ...
        for status_ in statuses:
            get_status[status_] = exception_schema \
                if status_ != status.HTTP_204_NO_CONTENT else {'model': None}
        return {**get_status, **self.get_base_statuses()}

    def __call__(self, schema=None,
                 response_status: int = status.HTTP_200_OK,
                 statuses: list = None):
        return self.statuses(schema=schema,
